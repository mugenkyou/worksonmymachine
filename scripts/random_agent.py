...
Runs all registered tasks choosing a random valid action at each step.
...
def run_random_task(task_alias: str, seed: int) -> Tuple[float, List[float]]:
    """
    Run a single task with the random agent.
    ...
    try:
        for step_index in range(1, task.max_steps + 1):
            ...
            action_command = random.choice(VALID_ACTIONS)
            action = Action(command=action_command)
...
def main() -> int:
    """Main entry point for the random baseline agent."""
    parser = argparse.ArgumentParser(
        description="Run TITAN tasks with random baseline agent."
    )
    ...
    for task_alias in tasks:
        score, _ = run_random_task(task_alias, seed=args.seed)
